        ...
        function ($scope, $element, $attrs, $compile, $parse, $document, $timeout, $uibPosition, dateFilter, uibDateParser, uiDatetimePickerConfig, $rootScope) {
            ...
            this.init = function(_ngModel) {
                ...
                var popupEl = angular.element('' +
                    ...
                    '<div uib-timepicker style="margin:0 auto" show-meridian="false"></div>' +
                    '</div>');
            ...
            $scope.getText = function (key) {
                ...
                {
                  case "now":
                    return $scope.nowText || uiDatetimePickerConfig.buttonBar[key].text;
                    break;
                  case "today":
                    return $scope.todayText || uiDatetimePickerConfig.buttonBar[key].text;
                    break;
                  case "clear":
                    return $scope.cancelText || uiDatetimePickerConfig.buttonBar[key].text;
                    break;
                  case "date":
                    return $scope.dateText || uiDatetimePickerConfig.buttonBar[key].text;
                    break;
                  case "time":
                    return $scope.timeText || uiDatetimePickerConfig.buttonBar[key].text;
                    break;
                  case "close":
                    return $scope.closeText || uiDatetimePickerConfig.buttonBar[key].text;
                    break;
                }
            };
            ...
            // Function used to show time popup
            $scope.openTime = function() 
            {
              /* The time is positioned according to the date */
              ...
              {
                var tpWidth = 188; //time picker width
                var tpHeight = 94; //time picker height+3px
                var posWidth = (position.width || 0) + 40; //40=calendar button

                switch($scope.popupPosition)
                {
                  case "up":
                  case "top-left":
                    $scope.dropdownStyle.top = -tpHeight + "px";
                    $scope.dropdownStyle.left = position.left + 'px';
                    break;
                  case "top-right":
                    $scope.dropdownStyle.top = -tpHeight + "px";
                    $scope.dropdownStyle.left = (posWidth-tpWidth) + "px";
                    break;
                  case "bottom-right":
            ...
            // Function used to show date popup
            $scope.openDate = function()
            {
              ...
              {
                var dpWidth = 265; //date picker width
                var dpHeight = 300; //date picker height+3px
                var posWidth = (position.width || 0) + 40; //40=calendar button
                ...
                switch ($scope.popupPosition)
                {
                  case "up":
                  case "top-left":
                    $scope.dropdownStyle.top = -dpHeight + "px";
                    $scope.dropdownStyle.left = position.left + 'px';
                    break;
                  case "top-right":
                    $scope.dropdownStyle.top = -dpHeight + "px";
                    $scope.dropdownStyle.left = (posWidth-dpWidth) + "px";
                    break;
                  case "bottom-right":
                    $scope.dropdownStyle.top = undefined;
                    $scope.dropdownStyle.left = (posWidth-dpWidth) + "px";
              ...
              if(position.width)
              {
                if(position.width < 265) //this is the date picker width
                  $scope.dropdownStyle.left = ((265 - position.width +10)/2) * -1 + 'px';
                else
                  $scope.dropdownStyle.left = position.left + 'px';
              }
              else
            ...
            /* Watch isOpen to decide what popup open */ 
            $scope.$watch('isOpen', function (value) {
                $scope.dropdownStyle = { display: value ? 'block' : 'none' };
                ...
                if (value) 
                {
                    ...
                    if($scope.enableDate)
                      $scope.openDate(); //Show the date popup
                    else
                      $scope.openTime(); //Show the time popup
                }
                else
...
angular.module('ui.bootstrap.datetimepicker').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('template/date-picker.html',
    "<ul class=\"dropdown-menu dropdown-menu-left datetime-picker-dropdown\" ng-if=\"isOpen && showPicker == 'date'\" ng-style=dropdownStyle style=left:inherit ng-click=$event.stopPropagation()><li style=\"padding:0 5px 5px 5px\" class=date-picker-menu><div ng-transclude></div></li><li style=padding:5px ng-if=buttonBar.show><span class=\"btn-group pull-left\" style=margin-right:10px ng-if=\"doShow('today') || doShow('clear')\"><button type=button class=\"btn btn-sm btn-default\" ng-if=\"doShow('time') && enableTime\" ng-click=\"openTime()\">{{ getText('time')}}</button><button type=button class=\"btn btn-sm btn-default\" ng-if=\"doShow('today')\" ng-click=\"select('today')\" ng-disabled=\"isDisabled('today')\">{{ getText('today') }}</button> </span> <span class=\"btn-group pull-right\" ng-if=\"(doShow('time') && enableTime) || doShow('close')\"> <button type=button class=\"btn btn-sm btn-default\" ng-if=\"doShow('clear')\" ng-click=\"select('clear')\">{{ getText('clear') }}</button><button type=button class=\"btn btn-sm btn-default\" ng-if=\"doShow('close')\" ng-click=close(true)>{{ getText('close') }}</button></span><span class=clearfix></span></li></ul>"
  );


  $templateCache.put('template/time-picker.html',
    "<ul class=\"dropdown-menu dropdown-menu-left datetime-picker-dropdown\" ng-if=\"isOpen && showPicker == 'time'\" ng-style=dropdownStyle style=left:inherit ng-click=$event.stopPropagation()><li style=\"padding:0 5px 5px 5px\" class=time-picker-menu><div ng-transclude></div></li><li style=padding:5px ng-if=buttonBar.show><span class=\"btn-group pull-left\" style=margin-right:10px ng-if=\"doShow('now') || doShow('clear')\"><button type=button class=\"btn btn-sm btn-default\" ng-if=\"doShow('date') && enableDate\" ng-click=\"openDate()\">{{ getText('date')}}</button> <button type=button class=\"btn btn-sm btn-default\" ng-if=\"doShow('now')\" ng-click=\"select('now')\" ng-disabled=\"isDisabled('now')\">{{ getText('now') }}</button> </span><span class=\"btn-group pull-right\" ng-if=\"(doShow('date') && enableDate) || doShow('close')\"><button type=button class=\"btn btn-sm btn-default\" ng-if=\"doShow('clear')\" ng-click=\"select('clear')\">{{ getText('clear') }}</button><button type=button class=\"btn btn-sm btn-default\" ng-if=\"doShow('close')\" ng-click=close(true)>{{ getText('close') }}</button></span> <span class=clearfix></span></li></ul>"
  );
