...
try
{
  _log = _logger.createLogger(
    _config.Log.path,
    _emConf.reader.logProperties.fileName,
    _emConf.reader.logProperties.category
  );
}
catch (err)
{
...
_httpSrv.start({config:_config, emConf:_emConf, log:_log},function(err)
{
  if (err)
  {
    _log.error("CANNOT start http server: " + err.message);
    process.exit(-1);
  }
});
